...
搜索和筛选针对完整目录，不只检查当前页。输入插件名称、插件 ID、仓库、分类、驱动或消息能力后，页面会在停止输入约 250 毫秒后更新结果；切换“动态”“静态”“已安装”“可更新”时会自动回到第一页。

筛选按钮旁的数字表示当前整份目录中各类插件的数量，不会随搜索词变化。目录标题右侧的“几个结果”才是搜索和筛选后的实际数量。

商城默认每页显示 20 个插件，也可以在列表底部改成 10 个或 50 个。底部分页栏会显示当前范围、结果总数和页码，使用左右箭头切换页面；到达第一页或最后一页后，对应箭头会自动禁用。修改每页数量会回到第一页。

普通搜索、筛选和翻页优先读取本地保存的目录，不需要反复访问 GitHub。需要同步刚合并的商城投稿时，再点击右上角“刷新目录”。选中插件后，右侧详情和完整版本记录会单独读取，网络或磁盘较慢时可能短暂显示加载状态。
...
读取静态目录
  ...
  -> 读取动态插件描述符
  -> 安全卸载现有动态库
  -> 用资产 SHA256 生成新的活动文件名
  -> 将旧活动文件移出 plugins/bin
  -> 重新扫描并执行 init
  -> 确认运行时版本并重建 Bot 分发器
  -> 成功后写入安装锁
```
...
商城管理的活动文件使用 `插件 ID + 完整资产 SHA256` 命名。同一个版本始终得到同一个文件名，不同二进制不会复用动态库路径。这项规则用于避开 Linux 动态加载器对旧 Rust/Tokio 动态库映像的延迟卸载；即使旧映像暂时仍由进程持有，新版本也会从独立路径加载。旧版 QimenBot 生成的固定文件名仍然有效，下一次更新或回滚时会自动迁移，不需要修改安装锁。
...
选择高于当前安装的兼容版本，点击“更新到此版本”。商城会保留上一版审核二进制，`plugins/bin` 中只放当前活动文件，因此运行时不会同时扫描到两个版本。面板在新插件完成 `init`、运行时报告版本一致且命令和事件分发器快照已刷新后，才会返回更新成功；已有 Bot 连接无需重建。

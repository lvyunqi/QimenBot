...
::: tip 静态与动态拦截器共存
静态插件的拦截器和动态插件的拦截器会合并到同一条链中执行：先执行静态拦截器，再执行动态拦截器。在 Web 插件页重新扫描时，动态拦截器会自动重建。
:::
...
::: tip 状态管理
拦截器实例在整个运行期间保持存在，可使用 `Mutex<HashMap<...>>` 保存冷却计时器、计数器等状态。拦截器可能被并发调用，内部状态必须满足线程安全要求。
:::

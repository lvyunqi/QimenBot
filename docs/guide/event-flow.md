...
|---------|------|------|
| 斜杠前缀 | `/ping` | 群聊和私聊 |
| 直接输入 | `ping` | 私聊和已开启“群聊直接输入”的群聊 |
| @提及 | `@Bot ping` | 群聊 |
| 回复消息 | (回复 Bot 消息) `ping` | 群聊和私聊 |

触发方式由 `[official_host.commands]` 控制，Web 面板入口是“配置 → 命令入口”。`group_bare_enabled` 默认开启，群聊中可以直接发送已注册的命令；关闭后群聊普通文本不会进入命令路由，只能使用前缀、@ 或回复等仍开启的入口。

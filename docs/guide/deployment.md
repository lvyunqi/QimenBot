...
| `QIMENBOT_TAG` | `latest` | 镜像版本；生产环境建议固定为 `0.1.20` 这类版本号 |
...
完整压缩包带有同名 `.sha256`。以下命令以 `v0.1.20` Linux x86_64 GNU 包为例：

```bash
sha256sum -c QimenBot-v0.1.20-x86_64-unknown-linux-gnu.tar.gz.sha256
```

...
- **中文命令无空格连写** — 支持如 `创建角色小明-男` 自动解析为命令 `创建角色` + 参数 `小明-男`
- **最长匹配优先** — 当存在 `创建` 和 `创建角色` 两个命令时，优先匹配更长的命令名
- **别名同样支持** — 命令别名也参与前缀匹配

### 动态插件生命周期

- **Runtime boot 自动调用 init** — 框架启动时自动遍历动态插件，读取 TOML 配置并转换为 JSON 传入 `#[init]`
- **TOML → JSON 配置桥接** — 插件配置文件 `config/plugins/<id>.toml` 自动转换

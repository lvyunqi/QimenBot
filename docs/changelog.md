...
- Runtime 删除 `echo`、`status`、`plugins`、`registry` 和 `dynamic-errors` 等硬编码命令及重复帮助清单；业务命令名完全交给静态或动态插件，停用插件后帮助目录不再残留旧命令。
- 宿主帮助改为插件匹配失败后的可关闭兜底，支持 `/help [页码]` 和每页数量配置；插件注册 `help`/`h` 时插件直接接管。
- 新增 `[official_host.commands]` 和 Web“命令入口”分区，可在线配置多个命令前缀、私聊裸命令、@ 提及、回复触发和帮助页大小；仅修改命令入口时 Bot 自动重连生效，无需重启宿主。

...
- Web 管理面板支持设置静态和动态插件的命令优先级，并按优先级、名称或类型排序；同名命令按管理员数值、插件声明顺序和插件 ID 稳定选择。
- 审计日志改为服务端分页，支持每页 `20`、`50` 或 `100` 条记录，并显示总数和当前范围。
- Runtime 删除 `ping`、`echo`、`status` 等硬编码业务命令及重复帮助清单；`/plugins`、`/registry` 和 `/dynamic-errors` 作为管理员宿主管理命令保留，以优先级 `10` 进入普通注册表，可在 Web 中逐项关闭，也可由更高优先级插件接管。
- 宿主帮助改为插件匹配失败后的可关闭兜底，支持 `/help [页码]` 和每页数量配置；插件注册 `help`/`h` 时插件直接接管。
- 新增 `[official_host.commands]` 和 Web“命令入口”分区，可在线配置多个命令前缀、私聊裸命令、@ 提及、回复触发、帮助页大小和三个宿主管理命令开关；仅修改命令入口时 Bot 自动重连生效，无需重启宿主。

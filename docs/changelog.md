...
> 以下内容为下一版本的预更新记录，正式版本号、发布日期和最终变更范围以发布结果为准。

### 动态插件生命周期与 CI

- 动态插件启用、停用、配置重载、商城安装/更新/回滚和卸载改为按插件 ID 的事务流程，只暂停并重新初始化目标动态库；无关插件不会重复执行 `shutdown` / `init`，已连接 Bot 直接读取新的分发器快照，无需断开连接。
- 动态插件回调超时后会保留可回收任务并隔离旧库，待回调真正返回后再释放动态库，避免后台线程持有旧版本导致更新卡住或“版本未变化”。
- 插件更新使用目标动态库路径和 SHA256 活动文件，加载失败会清理替换库、恢复旧文件并重新加载旧版本；动态回调超时不会强行卸载仍在执行的代码，而是返回可重试错误，避免出现“更新成功但实际仍是旧版本”。
- 管理端插件页新增动态插件在线卸载入口。卸载商城插件会沿用商城锁与回滚事务，卸载手工放入的动态库只移除活动二进制，配置和数据目录保留。
- 修复 GitHub Actions 在 Rust stable 新版 Clippy 中因固定 64 字节分块触发 `chunks_exact_to_as_chunks` 而失败的问题，同时保持 Rust 1.89 MSRV 可编译。
...
- 商城管理的动态插件改用“插件 ID + 完整资产 SHA256”作为活动文件名。新旧版本不再复用同一个 DLL、SO 或 dylib 路径，避免 GNU/Linux 因线程局部析构延迟卸载 Rust 动态库后，再次打开原路径仍取得旧版本映像。
- 文件事务支持在两个活动路径之间切换：更新时先移走旧文件，再原子写入新文件；初始化、运行时校验或安装锁保存失败时恢复原路径。旧版生成的固定活动文件名和安装锁继续兼容，下一次更新或回滚会自动迁移。
- 更新后的校验改为核对活动文件 SHA256 和宿主已经初始化的插件报告，不再对驻留中的动态库执行第二次描述符扫描。命令与事件分发器快照刷新后即可完成更新响应，已建立的 Bot 连接保持在线。
- 商城详情同时显示带本地时区的发布时间和安装时间；安装时间在插件完成加载与校验后写入。新增 GNU/Linux TLS 驻留动态库、旧文件名迁移、跨路径回滚和分发器代次测试。

### 管理面板与操作性能

- 动态插件目录扫描增加文件指纹缓存，并移到阻塞线程执行；插件较多时打开插件页、调整启停状态和修改优先级不再反复打开所有动态库，文件替换后会自动失效并重新扫描。
- 总览页消息吞吐图根据可用宽度自动调整时间刻度，保留起止时间并设置最小刻度间距，数据点增多时标签不会互相覆盖。

## v0.1.20 (2026-08-05)

### 命令入口与前缀

- 恢复可独立开关的“群聊直接输入”，默认允许群聊中直接发送已注册命令，无需前缀或 @ 机器人；配置文件使用 `official_host.commands.group_bare_enabled`，Web 面板保存后自动重连 Bot 生效。

### 插件库与文档

- 文档站新增独立的“插件库”入口，直接读取商城生成的真实索引，展示插件仓库、版本、驱动兼容、构建产物和 GitHub 来源。
- 插件库支持搜索、动态/静态筛选、详情面板和分页浏览；QQ 群入口移动到“相关链接”菜单，商城规范与插件库分开导航。
- 商城索引收录 `gpt-radar` 和 `newapi-status-bot` 两个真实插件，后续合并商城元数据后由文档流水线自动重新生成。

## v0.1.19 (2026-08-05)

### 命令系统与插件优先级

- Runtime 不再注册 `ping`、`echo`、`status` 等业务命令，也不再维护一份可能与插件状态不一致的固定帮助清单；业务命令和同名覆盖完全交给静态、动态插件决定。
- `/plugins`、`/registry` 和 `/dynamic-errors` 作为管理员宿主管理命令保留，以优先级 `10` 进入普通命令注册表。三项均可在 Web 中单独关闭，静态插件默认优先级 `30`、动态插件默认优先级 `20`，所以插件默认可以接管同名命令。
- Web 插件页支持为静态和动态插件设置 `0-1000` 的命令优先级，并按优先级、名称或类型排序。多个插件声明同名命令时，依次比较管理员优先级、插件声明顺序和插件 ID，重启后仍能得到相同结果。
- 宿主帮助改为插件匹配失败后的可关闭兜底，支持 `/help [页码]` 和每页数量配置。插件注册 `help` 或 `h` 后会直接接管，不会再与宿主帮助冲突。
- 新增 `[official_host.commands]` 和 Web“命令入口”分区，可在线配置多个命令前缀、私聊裸命令、@ 提及、回复触发、帮助页大小和三个宿主管理命令开关。保存后已启用的 Bot 自动重连并重建命令路由，无需重启宿主进程。

### Web 管理与审计

- 插件卡片增加命令优先级、当前排名和排序方式，优先级写入 `plugin-state.toml`；内置模块保持系统保留状态，避免误当成可覆盖的第三方插件。
- 审计日志改为服务端分页，支持每页 `20`、`50` 或 `100` 条记录，页面显示记录总数、当前范围和翻页状态，大量日志不再一次性载入浏览器。
- 配置页增加独立的“命令入口”表单，帮助分页和宿主管理命令使用直观开关，前缀、@、回复与私聊裸命令可以分别调整。

### 官方 QQ Bot 与运行时修复

- QQ 群和 C2C 本地富媒体分片上传同时兼容官方接口返回的从 `0` 或 `1` 开始的连续分片编号，并将原始编号完整传给 `upload_part_finish`，修复部分账号预上传成功后仍无法完成合并的问题。
- 整理 OneBot 11 反向 WebSocket 的会话退出路径，断开或失败后继续交由既有重连流程处理，同时消除新版 Clippy 的 `never_loop` 错误。

### 插件商城与开发文档

- 插件商城新增 API 0.6 社区插件条目，登记 OneBot 11、官方 QQ Bot、六个桌面与服务器 target、最低 glibc、SHA-256 和 GitHub 构建证明，商城可按现有兼容规则选择安装包。
- 插件开发 Skill 补充官方 QQ Bot Markdown 文档入口，分别说明静态插件、动态插件和运行排错时的格式边界；HTML 标签是否可用仍按群、C2C、频道和 DMS 的真实客户端结果验证。
- README、配置指南、运行时说明、Web 管理教程和动态插件模板统一改为插件拥有业务命令，并补齐优先级、分页帮助、宿主管理命令开关和重新扫描入口。

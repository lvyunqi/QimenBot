...
| 排序层级 | 规则 |
| --- | --- |
| 管理员命令优先级 | `0-1000`，数值越大越先匹配；静态插件默认 `30`，动态插件默认 `20` |
...
Runtime 不向注册表写入 `ping`、`echo`、`status` 等业务命令，也不会在注册表前抢占 `plugins`、`registry` 或 `dynamic-errors`。第三方插件可以自由使用这些名称。宿主 `help` 仅在插件匹配失败后兜底，因此插件声明 `help` 或 `h` 时仍由插件处理。

...
管理员优先级保存在 `plugin-state.toml` 的 `[priorities]` 表中。Web 面板保存后更新 Runtime 内存，并刷新共享的 `CommandDispatcher` 快照；已建立的 Bot 连接不需要重连。
...
管理员在 Web 插件页点击“重新扫描”，或调用 `POST /api/v1/plugins/reload`，才会执行全量重载：
...
插件页的启用、停用、配置 `reload`、商城安装/更新/回滚和卸载使用同一套按插件 ID 的事务流程：只封锁目标动态库路径，等待该插件的在途回调结束，执行 `shutdown`、解绑 Host API 和 `dlclose`，再扫描并初始化目标版本。无关插件不会重复执行 `init`，运行中的 Bot 会读取新的命令和事件分发器快照。动态回调超过超时后不能被 Rust 安全地强行终止；宿主会保留该库并返回失败，待回调结束后再重试，避免执行中的代码跳转到已卸载地址。

更新使用带版本和 SHA256 的活动文件路径。新库初始化成功后才替换运行时报告；失败会清理替换库、恢复旧文件并重新加载上一版本。停用只改变插件状态并卸载运行库，文件仍保留；在线卸载才会删除活动二进制，配置和数据目录不受影响。

...
QimenBot v0.1.10 为动态插件新增 API 0.4。插件后台线程可以显式选择 Bot，并在没有命令、事件或 Heartbeat 的情况下立即把发送请求交给宿主。
...
BotApi::for_bot 适合纯文本私聊、群聊和频道发送：
...
try_send 必须先调用 bot。未指定 Bot 会返回 InvalidRequest，不会选择最近事件 Bot 或任意默认 Bot。
...
- BotApi::for_bot 和 SendBuilder::bot(...).try_send()；
- 私聊、群聊、频道和频道私信目标；

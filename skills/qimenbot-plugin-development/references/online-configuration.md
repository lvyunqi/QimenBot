...
仓库内示例使用 path 依赖验证当前源码；独立插件应同时依赖两个 crates.io `0.1.13` 包，不要把 `api = "0.6"` 与 `0.1.12` 混用。需要核对线上版本时运行：
...
- 独立插件仓库使用两个同版本的正式 crates.io 依赖，不包含本地 QimenBot path、浮动 Git 分支、运行配置或动态库。

...
crate 发布版本与动态插件 ABI API 是两套版本。`0.1.12` 支持 API 0.1 至 0.5，并为 API 0.4/0.5 插件增加按 `account_id` 选择 Bot 的 Rust 接口。当前 API 0.5 是累积版本，同时包含实时主动发送和 Webhook；新插件统一推荐显式写出 `api = "0.5"`。`api = "0.4"` 继续兼容只使用主动发送的已有插件；未声明 `api` 时，过程宏仍生成兼容旧宿主的 API 0.3 插件。

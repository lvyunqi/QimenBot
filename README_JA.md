...
Runtime は `ping`、`echo`、`status` などの業務コマンドを登録しません。既定では管理者専用の `/plugins`、`/registry`、`/dynamic-errors` の 3 つだけをホストコマンドとして提供します。各コマンドは個別に無効化でき、通常の登録表優先度 `10` で処理されるため、静的プラグイン（`30`）と動的プラグイン（`20`）は既定で同名コマンドを上書きできます。任意のホスト help フォールバックは `/help [ページ]` に対応し、プラグインが `help` または `h` を登録した場合はプラグインが優先されます。

プレフィックス、プライベートチャットとグループチャットのプレフィックスなし入力、`@bot` メンション、返信トリガー、help の有効化、ページ件数、および 3 つのホストコマンドのスイッチは `[official_host.commands]` または Web 管理画面の「設定 → コマンド入口」で変更できます。グループチャットの直接入力は既定で有効ですが、プラットフォーム側でグループメッセージ全量イベントを受信できる必要があります。
...
│   ├── qimen-adapter-qqbot/ # 公式 QQ Bot アダプター
│   ├── qimen-transport-ws/  # WebSocket トランスポート
│   ├── qimen-transport-http/# HTTP トランスポート
│   ├── qimen-transport-qqbot/# 公式 QQ Bot Gateway/OpenAPI

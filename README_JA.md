...
Runtime は業務コマンドを登録しません。`ping`、`echo`、`status`、`plugins` などは、静的または動的プラグインが登録した場合にのみ応答します。任意のホスト help フォールバックは `/help [ページ]` に対応し、プラグインが `help` または `h` を登録した場合はプラグインが優先されます。

プレフィックス、プライベートチャットのプレフィックスなし入力、`@bot` メンション、返信トリガー、help の有効化、ページ件数は `[official_host.commands]` または Web 管理画面の「設定 → コマンド入口」で変更できます。
